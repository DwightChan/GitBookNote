...
- [案例-登录认证](#案例-登录认证)
  ...
  - [2. 登录校验](#2-登录校验)
    - [2.1 问题分析](#21-问题分析)
    - [2.2 会话技术](#22-会话技术)
      - [2.2.1 会话技术介绍](#221-会话技术介绍)
      - [2.2.2 会话跟踪方案](#222-会话跟踪方案)
        - [2.2.2.1 方案一 - Cookie](#2221-方案一---cookie)
        - [2.2.2.2 方案二 - Session](#2222-方案二---session)
        - [2.2.2.3 方案三 - 令牌技术](#2223-方案三---令牌技术)
    - [2.3 JWT令牌](#23-jwt令牌)
      - [2.3.1 介绍](#231-介绍)
      - [2.3.2 生成和校验](#232-生成和校验)
      - [2.3.3 登录下发令牌](#233-登录下发令牌)
    - [2.4 过滤器Filter](#24-过滤器filter)
      - [2.4.1 快速入门](#241-快速入门)
      - [2.4.2 Filter详解](#242-filter详解)
        - [2.4.2.1 执行流程](#2421-执行流程)
        - [2.4.2.2 拦截路径](#2422-拦截路径)
        - [2.4.2.3 过滤器链](#2423-过滤器链)
      - [2.4.3 登录校验-Filter](#243-登录校验-filter)
        - [2.4.3.1 分析](#2431-分析)
        - [2.4.3.2 具体流程](#2432-具体流程)
        - [2.4.3.3 代码实现](#2433-代码实现)
    - [2.5 拦截器Interceptor](#25-拦截器interceptor)
      - [2.5.1 快速入门](#251-快速入门)
      - [2.5.2 Interceptor详解](#252-interceptor详解)
        - [2.5.2.1 拦截路径](#2521-拦截路径)
        - [2.5.2.2 执行流程](#2522-执行流程)
      - [2.5.3 登录校验- Interceptor](#253-登录校验--interceptor)
  - [3. 异常处理](#3-异常处理)
    - [3.1 当前问题](#31-当前问题)
    - [3.2 解决方案](#32-解决方案)
    - [3.3 全局异常处理器](#33-全局异常处理器)
...
- 第一部分：Header(头）， 记录令牌类型、签名算法等。 例如：{"alg":"HS256","type":"JWT"} （通过base64 编码得到）

- 第二部分：Payload(有效载荷），携带一些自定义信息、默认信息等。 例如：{"id":"1","username":"Tom"} （通过base64 编码得到）

- 第三部分：Signature(签名），防止Token被篡改、确保安全性。将header、payload，并加入指定秘钥，通过指定签名算法计算而来。（通过签名得到）
...
public class WebConfig implements WebMvcConfigurer {
    ...
    //自定义的拦截器对象
    @Autowired // 这个注解 是Configuration 自动注入
