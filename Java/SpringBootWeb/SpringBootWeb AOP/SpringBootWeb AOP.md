- [事务\&AOP](#事务aop)
  - [1. 事务管理](#1-事务管理)
    - [1.1 事务回顾](#11-事务回顾)
    - [1.2 Spring事务管理](#12-spring事务管理)
      - [1.2.1 案例](#121-案例)
      - [1.2.2 原因分析](#122-原因分析)
      - [1.2.3 Transactional注解](#123-transactional注解)
    - [1.3 事务进阶](#13-事务进阶)
      - [1.3.1 rollbackFor](#131-rollbackfor)
      - [1.3.3 propagation](#133-propagation)
        - [1.3.3.1 介绍](#1331-介绍)
        - [1.3.3.2 案例](#1332-案例)
  - [2. AOP基础](#2-aop基础)
    - [2.1 AOP概述](#21-aop概述)
    - [2.2 AOP快速入门](#22-aop快速入门)
    - [2.3 AOP核心概念](#23-aop核心概念)
  - [3. AOP进阶](#3-aop进阶)
    - [3.1 通知类型](#31-通知类型)
    - [3.2 通知顺序](#32-通知顺序)
    - [3.3 切入点表达式](#33-切入点表达式)
      - [3.3.1 execution](#331-execution)
      - [3.3.2 @annotation](#332-annotation)
    - [3.4 连接点](#34-连接点)
  - [4. AOP案例](#4-aop案例)
    - [4.1 需求](#41-需求)
    - [4.2 分析](#42-分析)
...
public class DeptServiceImpl implements DeptService {
    ...
    @Override
    @Transactional(rollbackFor=Exception.class) // 表示所有异常都执行回滚
    public void delete(Integer id){
        //根据部门id删除部门信息
        deptMapper.deleteById(id);
...
public class TimeAspect {
    ...
    @Around("execution(* com.itheima.service.*.*(..))")  // 切入点表达式 表示指定 在 service 下面的所有类 所有的方法
...
public class MyAspect1 {
    //前置通知 
    // 第一个 * 表示返回值为任意类型，
    // 第二个 * 表示 com.itheima.service 包下的任意类 ， 
    // 第三个 * 表示 该类下的任意 方法；
...
~~~java
@Target(ElementType.METHOD) // 注解使用对象 在方法上
@Retention(RetentionPolicy.RUNTIME) // 注解 生效在运行时
...
public class LogAspect {
    ...
    // 这里直接 注入 请求 对象 在下面可以直接拿到每次的请求 头信息；
    @Autowired
